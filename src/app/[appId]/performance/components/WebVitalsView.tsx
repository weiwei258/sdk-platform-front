...
export const WebVitalsView = (props: WebVitalsViewProps) => {
  ...
  const map = useMemo<WebVitals>(() => {
    ...
    data.forEach((item) => {
      if (item.category === TransportCategory.PERF) {
        ...
        if (item.context.CLS) {
          CLSManager.sum += 1
          CLSManager.totalTime += item.context.CLS.value
        }
        if (item.context.FID) {
          FIDManager.sum += 1
          FIDManager.totalTime += item.context.FID.value
        }
        if (item.context.LCP) {
          LCPManager.sum += 1
          LCPManager.totalTime += item.context.LCP.value
        }
        if (item.context.LCP) {
          FCPManager.sum += 1
          FCPManager.totalTime += item.context.LCP.value
        }
        if (item.context.LCP) {
          FPManager.sum += 1
          FPManager.totalTime += item.context.LCP.value
        }
      }
  ...
  return (
    <Row>
      ...
      <Col span={8}>
        FCP：{map.FCP.averageTime}ms
      </Col>
      <Col span={8}>
        FP：{map.FCP.averageTime}ms
      </Col>
    </Row>
  )
}
